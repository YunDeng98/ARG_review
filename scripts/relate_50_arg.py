...
relate_main_cmd = f"../../relate_v1.2.1_x86_64_static/bin/Relate --mode All -m {mutation_rate} -N {2*pop_size} --haps sim_50_{index}.haps --sample sim_50_{index}.sample --map 50_{index}_map_file.txt --seed 1 -o relate_50_{index}"

subprocess.run(relate_main_cmd, shell=True)

relate_convert_ts = f"../../relate_v1.2.1_x86_64_static/bin/RelateFileFormats --mode ConvertToTreeSequence -i relate_50_{index} -o relate_50_{index}"

subprocess.run(relate_convert_ts, shell=True)

clean_cmd = f"../../relate_v1.2.1_x86_64_static/bin/RelateFileFormats/bin/Relate --mode Clean -o relate_50_{index}"
